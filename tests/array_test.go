...
func TestStringArrayWithInvalidSubtypes(t *testing.T) {
	...
	assert.ElementsMatch(t, []int64{0, 0, 0}, result["array_int"])
	assert.ElementsMatch(t, []uint64{0, 0, 0}, result["array_uint"])
...
func TestMixTypesArray(t *testing.T) {
	...
	assert.ElementsMatch(t, []int64{1, 0, 5}, result["array_int"])
	assert.ElementsMatch(t, []uint64{1, 0, 5}, result["array_uint"])
	assert.ElementsMatch(t, nil, result["array_binary"])
	assert.ElementsMatch(t, nil, result["array_float"])
	assert.ElementsMatch(t, nil, result["array_bool"])
	var tt time.Time
